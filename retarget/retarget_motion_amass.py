...
if __name__ == '__main__':
    ...
    for motions_folder in motions_folders:
        print(f"handling motion folder {motions_folder} ...")
        if not os.path.exists(os.path.join(output_folder, motions_folder)):
            os.mkdir(os.path.join(output_folder, motions_folder))
        for motion in os.listdir(os.path.join(dataset_folder, motions_folder)):  
            if motion.endswith("stagei.npz"): continue
            print(f"handling motion {motion} ...") 
            output, file_name = process_file(
                file_path=os.path.join(dataset_folder, motions_folder, motion),
                target_fps=60,
